...
- **Known cleanup (deferred — bigger than it looks, leave it):** the working tree shows ~1,271 LF/CRLF "phantom" files, but the line-ending inconsistency is actually **repo-wide** — a `* text=auto eol=lf` + `git add --renormalize` stages **~16,028 files (4,547 in `src/matched/`)**. A full normalization is therefore a MAJOR dedicated operation: a huge diff that triggers the pre-commit hook to re-verify all 4,547 matches (they pass — EOL doesn't change compiled bytes — but it takes hours and must be batched or the hook temporarily set aside). **NOT a quick fix.** Recommendation: **leave the phantom** — it's benign (empty-diff, never staged, neutralized by the scoped-`--only` commit rule). Only renormalize as a deliberate standalone pass with the hook handled, if it ever actually causes a problem (it hasn't).
...
> You run in WSL and CANNOT commit (drvfs wedge) — STAGE only, DM MainGuy `commit-ready: <paths> — <msg>`. **Flagship: a real instruction-level objdiff view.** The whole 392B+ GeneralAllocator tier + many apt/point-version walls are decoded + parked in `src/wip/near_match/` as register-coloring/redundant-mr/scheduling near-matches — they need objdiff to (a) confirm them as permanent compiler-version walls or (b) reveal a source lever. Extend `diff_func.sh`/`disasm_digest.sh` or integrate objdiff into a clean offset-aligned side-by-side that highlights coloring vs ordering vs count diffs. **HEAD START:** `objdiff.json` is ALREADY configured in the repo root (encounter/objdiff schema v2, `custom_make: ninja`, `build_base: true`, game/sdk progress categories, watch_patterns set). The TOOL just needs installing — download the objdiff app/binary from `github.com/encounter/objdiff/releases` (GUI app or `objdiff-cli`), or `cargo install objdiff-cli` if cargo is available (it currently is NOT on this machine). So your job is **install → run against the parked `src/wip/near_match/` decodes → extend**, not configure from scratch. Tooling only — never author matches; tools must NEVER enable spoofing (no byte injection, verify suppression, or asm rewrite).
