// 0x80295A30 AptDate::sMethod_setMilliseconds(AptValue*, int) (256B) — clean
//
// AptScript native handler: AptDate.setMilliseconds(v). With no argument, returns the
// cached "undefined" value. Otherwise pops the top argument off the AptScript
// argument stack, coerces it to an integer, stores it as field 0x40, recomputes
...
struct AptDate {
    ...
    static AptValue* sMethod_setMilliseconds(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setMilliseconds(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    int count = g_aptArgStack.count;
    AptValue* arg = g_aptArgStack.data[count - 1];
    *(int*)((char*)self + 0x40) = arg->toInteger();
