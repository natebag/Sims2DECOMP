// 0x80295D30 AptDate::sMethod_setSeconds(AptValue*, int) (256B) — clean
//
// AptScript native handler: AptDate.setSeconds(v). With no argument, returns the
// cached "undefined" value. Otherwise pops the top argument off the AptScript
// argument stack, coerces it to an integer, stores it as field 0x24, recomputes
...
struct AptDate {
    ...
    static AptValue* sMethod_setSeconds(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setSeconds(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    int count = g_aptArgStack.count;
    AptValue* arg = g_aptArgStack.data[count - 1];
    *(int*)((char*)self + 0x24) = arg->toInteger();
