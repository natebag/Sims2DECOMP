// 0x802957E0 AptDate::sMethod_setFullYear(AptValue*, int) (336B) — clean
//
// AptScript native handler: AptDate.setFullYear(year[, month[, day]]). With no
// argument returns the cached "undefined" value. Otherwise pops up to three
// arguments off the AptScript argument stack (top=year -> field 0x3C, then
// month -> 0x38 if argc>1, then day -> 0x34 if argc>2), coercing each to an
// integer, recomputes the cached timestamps via setDates (local convention),
// then boxes 0 into a number AptValue via the inlined AptValueGC allocator.
//
// Each later arg's index is held in a named intermediate (count-1, count-2) so
// SN ProDG subtracts before scaling (addi;slwi;lwz -4) rather than folding the
// offset into the displacement.
...
struct AptDate {
    ...
    static AptValue* sMethod_setFullYear(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setFullYear(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    *(int*)((char*)self + 0x3C) = g_aptArgStack.data[g_aptArgStack.count - 1]->toInteger();
    if (argc > 1) {
        int i2 = g_aptArgStack.count - 1;
        *(int*)((char*)self + 0x38) = g_aptArgStack.data[i2 - 1]->toInteger();
    }
    if (argc > 2) {
        int i3 = g_aptArgStack.count - 2;
        *(int*)((char*)self + 0x34) = g_aptArgStack.data[i3 - 1]->toInteger();
    }
