// 0x80296448 AptDate::sMethod_setUTCMonth(AptValue*, int) (260B) — clean
//
// AptScript native handler: AptDate.setUTCMonth(v). With no argument, returns the
// cached "undefined" value. Otherwise pops the top argument off the AptScript
// argument stack, coerces it to an integer, stores it as field 0x58, recomputes
...
struct AptDate {
    ...
    static AptValue* sMethod_setUTCMonth(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setUTCMonth(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    int count = g_aptArgStack.count;
    AptValue* arg = g_aptArgStack.data[count - 1];
    *(int*)((char*)self + 0x58) = arg->toInteger();
