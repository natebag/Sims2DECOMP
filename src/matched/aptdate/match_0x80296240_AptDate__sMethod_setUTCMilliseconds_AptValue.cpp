// 0x80296240 AptDate::sMethod_setUTCMilliseconds(AptValue*, int) (260B) — clean
//
// AptScript native handler: AptDate.setUTCMilliseconds(v). With no argument, returns the
// cached "undefined" value. Otherwise pops the top argument off the AptScript
// argument stack, coerces it to an integer, stores it as field 0x60, recomputes
// the cached timestamps via setDates with the UTC convention (clocks swapped vs
// the local setters and the offset negated: clockA=self+0x44, clockB=self+0x24,
...
struct AptDate {
    ...
    static AptValue* sMethod_setUTCMilliseconds(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setUTCMilliseconds(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    int count = g_aptArgStack.count;
    AptValue* arg = g_aptArgStack.data[count - 1];
    *(int*)((char*)self + 0x60) = arg->toInteger();
