// 0x8029654C AptDate::sMethod_setUTCSeconds(AptValue*, int) (260B) — clean
//
// AptScript native handler: AptDate.setUTCSeconds(v). With no argument, returns the
// cached "undefined" value. Otherwise pops the top argument off the AptScript
// argument stack, coerces it to an integer, stores it as field 0x44, recomputes
...
struct AptDate {
    ...
    static AptValue* sMethod_setUTCSeconds(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setUTCSeconds(AptDate* self, int argc) {
    if (argc <= 0) {
        return g_aptDateUTCResult;
    }
    int count = g_aptArgStack.count;
    AptValue* arg = g_aptArgStack.data[count - 1];
    *(int*)((char*)self + 0x44) = arg->toInteger();
