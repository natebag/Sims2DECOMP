// 0x80295E30 AptDate::sMethod_setTime(AptValue*, int) (180B) — clean
//
// AptScript native handler: boxes the constant 0 into a number AptValue (the
// "setTime" stub returns 0). Byte-identical structure to getTime — inlined
// AptValueGC allocator (recycle off the GC free-list, else allocate fresh); the
// allocate path reuses the (null) free-list register as the zero store.
...
struct AptDate {
    static AptValue* sMethod_setTime(AptDate* self, int argc);
};

AptValue* AptDate::sMethod_setTime(AptDate* self, int argc) {
    int value = 0;
