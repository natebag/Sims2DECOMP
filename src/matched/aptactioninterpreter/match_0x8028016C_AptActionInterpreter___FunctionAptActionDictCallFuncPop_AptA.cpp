// 0x8028016C AptActionInterpreter::_FunctionAptActionDictCallFuncPop(
...
// working stack A0 and dispatches its per-type method (vtable+0x08, slot 1, arg 138),
// chains the CallFunction opcode handler, then — if the operand stack is non-empty —
// dispatches the top-of-stack box's second per-type method (vtable+0x10, slot 2,
// arg 160) and pops it. Finally releases the GC value vector if it holds values and
// the stack has drained.
//
// The shared gAptGlobalB pointer feeds BOTH dispatches, so its base stays in a
// callee-saved register across CallFunction; gAptGlobalA / gAptGlobalA2 differ.
...
extern AptGlobal gAptDCFPGlobalA;
extern AptGlobal gAptDCFPGlobalB;
extern AptGlobal gAptDCFPGlobalA2;
...
struct AptActionInterpreter {
    ...
    static void _FunctionAptActionDictCallFuncPop(AptActionInterpreter* interp,
                                                  LocalContextT* ctx);
    static void _FunctionAptActionCallFunction(AptActionInterpreter*, LocalContextT*);
};

void AptActionInterpreter::_FunctionAptActionDictCallFuncPop(AptActionInterpreter* interp,
                                                             LocalContextT* ctx) {
    ...
    box->Dispatch(&gAptDCFPGlobalA, &gAptDCFPGlobalB, 138);
    ...
    if (n > 0) {
        ...
        tos->Dispatch2(&gAptDCFPGlobalA2, &gAptDCFPGlobalB, 160);
