// 0x8001ADB8 ESimsCam::ReadControllerZoom(void) (84B)
...
public:
    ...
    int ReadControllerZoom();
};

int ESimsCam::ReadControllerZoom() {
    EControllerManager* mgr = gv_controllerMgr;
    int pIdx = mgr->GetPlayerControllerIndex(m_playerIdx);
    return mgr->GetController(pIdx)->GetStick(1, 1, m_field1376);
}
