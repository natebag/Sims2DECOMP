// 0x803C2DA8 TArray<EString2, TArrayDefaultAllocator>::CopyReverse(EString2*, EString2*, int) (104 B)
...
struct EString2 { wchar_t* ptr; };
extern void EString2_op_assign(EString2* dst, wchar_t* src);

void TArray_EString2_CopyReverse(EString2* dst, EString2* src, int count) {
    int i = count - 1;
    if (count == 0) return;
    int offset = (i * 4);
    dst = (EString2*)((char*)dst + offset);
    src = (EString2*)((char*)src + offset);
    do {
        EString2_op_assign(dst, src->ptr);
        dst--; src--;
    } while (i-- != 0);
}
