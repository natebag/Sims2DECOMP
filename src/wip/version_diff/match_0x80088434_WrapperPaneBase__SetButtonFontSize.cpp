/* 0x80088434 (96 bytes) - WrapperPaneBase::SetButtonFontSize(WrapperPaneBase::ButtonType, float) */

typedef int ButtonType;

class TextBlock {
public:
    char _pad_0[0x34];
    float m_fontSize;
    void SetDirty(void);
};

class TextBaseItem {
public:
    char _pad_0[4];
    unsigned int m_flags;
    char _pad_8[0x18];
    TextBlock m_textBlock;
};

class WrapperPaneBase {
public:
    char _pad_0[0x40];
    TextBaseItem *m_buttons[3];

    void UpdateAptButtonWidth(ButtonType type);
    void SetButtonFontSize(ButtonType type, float size);
};

void WrapperPaneBase::SetButtonFontSize(ButtonType type, float size)
{
    TextBaseItem *btn = m_buttons[type];
    TextBlock *tb = &btn->m_textBlock;
    tb->m_fontSize = size;
    tb->SetDirty();
    btn->m_flags |= 0x02;
    UpdateAptButtonWidth(type);
}
