#!/usr/bin/env python3
"""
Full 64-128B target scan across ALL systems.
Parses DVD map, excludes already-matched functions, excludes SDK range.
Outputs grouped list to tools/all_64_128b_targets.txt
"""
import re
import json
...
OUTPUT_PATH = PROJECT_ROOT / "tools/all_64_128b_targets.txt"
JSON_PATH = PROJECT_ROOT / "tools/all_64_128b_targets.json"
...
def main():
    ...
    print("Parsing DVD map for size 64-128 functions...")
    entries = parse_dvd_map()
    print(f"  Total 64-128B entries in map (excl. SDK): {len(entries)}")
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("FULL 64-128B TARGET SCAN — ALL SYSTEMS")
    lines.append(f"Map: {MAP_PATH.name}")
    lines.append(f"Total 64-128B functions in map (SDK excluded): {len(entries)}")
    lines.append(f"Already matched (src/matched + src/wip): {len(all_matched)}")
    lines.append(f"UNMATCHED 64-128B TARGETS: {len(unmatched)}")
    lines.append("=" * 70)
    lines.append("")

    json_data = {
        "total_64_128b_in_map": len(entries),
