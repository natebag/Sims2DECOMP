#!/usr/bin/env python3
"""Wave-4 batch converter: agent inject_before stubs -> class-structured semantic .cpp"""
import os, re, shutil
...
for cls in sorted(class_files.keys()):
    ...
    if not os.path.isdir(dir_path):
        skipped_no_dir += len(class_files[cls])
        continue
    ...
    for fname in class_files[cls]:
        ...
        for line in lines:
            ...
            if stripped.startswith('// 0x') or stripped.startswith('// FLAGS:') or stripped.startswith('// ASMPROC'):
                ...
                # Only keep helper declarations like: extern "C" void _sADDR_N();
                # Do NOT keep the old stub: extern "C" void f_ADDR() {}
...
print('Skipped no dir: ' + str(skipped_no_dir))

# Write pathspec files
with open('.git/sw1_wave4_add.txt', 'w', encoding='utf-8', newline='\n') as fh:
    fh.write('\n'.join(add_list) + '\n')

with open('.git/sw1_wave4_rm.txt', 'w', encoding='utf-8', newline='\n') as fh:
    fh.write('\n'.join(rm_list) + '\n')

print('')
print('Pathspec files written:')
print('  .git/sw1_wave4_add.txt (' + str(len(add_list)) + ' files)')
print('  .git/sw1_wave4_rm.txt (' + str(len(rm_list)) + ' files)')
