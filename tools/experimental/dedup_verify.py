...
def verify(f):
    ...
    if not m:
        return None, 'no size'
    size = m.group(1)
    result = subprocess.run(['bash', 'tools/verify_match.sh', f, addr, size], capture_output=True, text=True, timeout=30)
    return result.returncode == 0, result.stdout[-200:] if len(result.stdout) > 200 else result.stdout
