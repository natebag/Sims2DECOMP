...
def has_sda_content(path):
    try:
        ...
        patterns = [
            r'extern\s+char\s+\w+',
            r'extern\s+unsigned\s+char\s+\w+',
            r'extern\s+short\s+\w+',
            r'extern\s+unsigned\s+short\s+\w+',
            r'extern\s+int\s+\w+',
            r'extern\s+unsigned\s+int\s+\w+',
            r'extern\s+float\s+\w+',
            r'extern\s+double\s+\w+',
            r'extern\s+ETypeInfo\s+\w+',
...
# Scan all matched files for SDA patterns
...
# Build reports
print("=== SDA-DEPENDENT METHOD FAMILIES ===\n")
method_families = []
...
print(f"Found {len(method_families)} method families")
for fam in method_families[:25]:
...
print(f"Found {len(class_families)} class families")
...
# Save
with open('build/reports/sda_family_report5.json', 'w') as fp:
