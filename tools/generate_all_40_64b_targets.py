#!/usr/bin/env python3
"""
Full 40-64B target scan across ALL systems.
Parses DVD map, excludes already-matched functions, excludes SDK range.
Outputs grouped list to tools/all_40_64b_targets.txt
"""
import re
import json
...
OUTPUT_PATH = PROJECT_ROOT / "tools/all_40_64b_targets.txt"
JSON_PATH = PROJECT_ROOT / "tools/all_40_64b_targets.json"
    ...
    with MAP_PATH.open("r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if 0x80240000 <= addr <= 0x8039FFFF:
                continue
            symbol = rest.strip()
            # Filter out file paths that may contain parens like .lib(foo.obj)
...
def main():
    ...
    print("Parsing DVD map for size 40-64 functions...")
    entries = parse_dvd_map()
    print(f"  Total 40-64B entries in map (excl. SDK): {len(entries)}")
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("FULL 40-64B TARGET SCAN — ALL SYSTEMS")
    lines.append(f"Map: {MAP_PATH.name}")
    lines.append(f"Total 40-64B functions in map (SDK excluded): {len(entries)}")
    lines.append(f"Already matched (src/matched + src/wip): {len(all_matched)}")
    lines.append(f"UNMATCHED 40-64B TARGETS: {len(unmatched)}")
    lines.append("=" * 70)
    lines.append("")

    json_data = {
        "total_40_64b_in_map": len(entries),
