"""Analyze non-BLR unmatched functions: check if try_gen_stub_lines would succeed."""
import struct, re, glob, sys

# Copy insn_asm_extended and try_gen_stub_lines from gen_stubs inline to avoid running it
...
def insn_asm_extended(w):
    ...
    if op == 14:
        ...
        da_sel = {23: 'fsel'}
        if xop10 in d0_map:
            bf = (w >> 23) & 0x7; return '{} {},f{},f{}'.format(d0_map[xop10], bf, fra, frb)
        elif xop10 in d_map:
...
def insn_asm_extended_or_long(w):
    ...
    return r if r is not None else '.long 0x{:08X}'.format(w)
...
def try_gen_stub_lines(addr, body_words):
    ...
    for i, w in enumerate(body_words):
        ...
        if op == 18:
            ...
            mn, cr_str = result; target_idx = i + bd
            if 0 <= target_idx <= n:
    ...
    for i, w in enumerate(body_words):
        ...
        if i in bl_stubs:
            ...
            info = br_info[i]
            t = info['target']; lnum = target_to_label[t]
...
# Now analyze non-BLR unmatched functions
cat1_loop = []   # last b targets within body
cat2_extern = [] # last b targets outside body
cat1_ok = []     # cat1 where try_gen_stub_lines succeeds
cat2_ok = []     # cat2 where try_gen_stub_lines succeeds (on body_words[:-1])
...
for addr, (size, name) in sorted(syms.items()):
    ...
    if last_op != 18 or last_lk != 0:
        continue  # only handle plain b

    li = (last_w >> 2) & 0xFFFFFF
    if li >= 0x800000: li -= 0x1000000
    last_addr = addr + size - 4
    target_abs = last_addr + li * 4
    within = (addr <= target_abs < addr + size)

    if within:
        cat1_loop.append((addr, size, name))
        result = try_gen_stub_lines(addr, list(words))
        if result is not None:
            cat1_ok.append((addr, size, name))
    else:
        cat2_extern.append((addr, size, name, target_abs))
        result = try_gen_stub_lines(addr, list(words[:-1]))
        if result is not None:
            cat2_ok.append((addr, size, name, target_abs))

print(f'Category 1 (loop/within-body b): {len(cat1_loop)} total, {len(cat1_ok)} gen_stubs-ready')
print(f'Category 2 (external tail-call): {len(cat2_extern)} total, {len(cat2_ok)} gen_stubs-ready')
print()
print(f'Cat1 ready:')
for addr, size, name in cat1_ok[:30]:
    print(f'  0x{addr:08X} {size:4d}B {name[:70]}')
if len(cat1_ok) > 30: print(f'  ... and {len(cat1_ok)-30} more')
print()
print(f'Cat2 ready:')
for addr, size, name, tgt in cat2_ok[:30]:
    tname = syms.get(tgt, (0, '???'))[1]
    print(f'  0x{addr:08X} {size:4d}B -> 0x{tgt:08X} ({tname[:30]})  {name[:40]}')
if len(cat2_ok) > 30: print(f'  ... and {len(cat2_ok)-30} more')
